...
pipeline {
  ...
  stages {
    stage('Checkout') {
      steps {
        git branch: 'main', url: 'https://github.com/MohanGit1990/DevSecOps-Project1.git'
      }
    }
   ...
   stage('Stage VII: Scan Image ') {
      steps { 
        ...
        sh "trivy image --scanners vuln --offline-scan mohankumar1990/democicd:latest > trivyresults.txt"
        }
    }
   ...
   stage('Stage VIII: Smoke Test ') {
      steps { 
        ...
        sh "docker run -d --name smokerun -p 8080:8080 mohankumar1990/democicd"
